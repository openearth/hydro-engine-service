...
from hydroengine_service.cache import cache
...
# then register the blueprints, so that they can also use the cache
...
bathymetry = {
    ...
    "vaklodingen": ee.ImageCollection(
        "projects/deltares-rws/eo-bathymetry/vaklodingen"
    ),
    "maasvlakte": ee.ImageCollection("projects/deltares-rws/eo-bathymetry/maasvlakte"),
...
def get_upstream_catchments(level):
    ...
    def _get_upstream_catchments(basin_source) -> ee.FeatureCollection:
        ...
        upstream_ids = index.filter(ee.Filter.eq("hybas_id", hybas_id)).aggregate_array(
            "parent_from"
        )
        upstream_basins = (
            basins[level]
            .filter(ee.Filter.inList("HYBAS_ID", upstream_ids))
            .merge(ee.FeatureCollection([basin_source]))
        )
...
def api_get_image_urls():
    ...
    dataset = r[
        "dataset"
    ]  # bathymetry_jetski | bathymetry_vaklodingen | dem_srtm | ...
    ...
    rasters = {
        ...
        "bathymetry_maasvlakte": bathymetry["maasvlakte"],
    ...
    def generate_image_info(image):
        ...
        return {"mapid": mapid, "url": url, "begin": begin, "end": end}
...
def get_sea_surface_height_time_series():
    ...
    return Response(
        json.dumps(ssh_rows.getInfo()), status=200, mimetype="application/json"
    )
...
def get_sea_surface_height_trend_image():
    ...
    response = Response(
        json.dumps({"url": url}), status=200, mimetype="application/json"
    )
...
def hillshade(image_rgb, elevation, reproject, height_multiplier=500, weight=1.2):
    ...
    if reproject:
        z = elevation.reproject(ee.Projection("EPSG:3857").atScale(30)).multiply(
            ee.Image.constant(height_multiplier)
        )
    else:
...
def api_get_bathymetry():
    ...
    palettes = {
        ...
        "maasvlakte": """#9900FF,#9900FF,#8810FF,#7621FF,#6532FF,#4354FF,#3265FF,#2176FF,#0099FF,#1AA4FF,#35AFFF,#6BC5FF,#86D0FF,#A1DBFF,#D7F1FF,#D9E0A3,#F3CA89,#E2B247,#B89E21,#A49018""",
    ...
    def sorted_composite(images):
        ...
        sorted_composite = sorted_images.reduce(ee.Reducer.firstNonNull())
        # store metadata
    ...
    def generate_image_info(image):
        ...
        image_vis = image.visualize(
            **{"min": image_min, "max": image_max, "palette": image_palette}
        )
        ...
        if "hillshade" in r and r["hillshade"]:
            image_vis = hillshade(
                image_vis,
                image.subtract(image_min).divide(
                    ee.Image.constant(image_max).subtract(image_min)
                ),
                True,
            )
...
def get_water_mask_raw():
    ...
    water_mask = image.normalizedDifference().gt(ndwi_threshold)

    # vectorize
    water_mask_vector = water_mask.mask(water_mask).reduceToVectors(
        **{"geometry": region, "scale": scale / 2}
    )

    water_mask_vector = water_mask_vector.toList(10000).map(
        lambda o: ee.Feature(o).simplify(scale)
    )
...
def get_water_mask_vector(region, scale, start, stop):
    ...
    water_occurrence = (
        monthly_water.filterDate(start, stop)
    ...
    # clean-up
    water_mask = water_mask.focal_mode(scale * 3, "circle", "meters")

    # vectorize
    water_mask_vector = water_mask.mask(water_mask).reduceToVectors(
        **{"geometry": region, "scale": scale / 2, "tileScale": 4}
    )

    # take the largest
    water_mask_vector = water_mask_vector.map(lambda o: o.set({"area": o.area(scale)}))

    water_mask_vector = ee.Feature(water_mask_vector.sort("area", False).first())
...
def transform_feature(crs, scale):
    return lambda f: f.transform(
        ee.Projection(crs).atScale(scale), ee.Number(scale).divide(100)
    )
...
def get_water_network_properties():
    ...
    if "network" in j:
        raise Exception("TODO: re-using existing networks is not supported yet")
    ...
    centerline = centerline.map(lambda line: line.set("length", line.length(error)))
    ...
    def process_long_line(line):
        ...
        def generate_line_middle_point(pair):
            ...
            return ee.Feature(centroid).set("lineId", line.id()).set("offset", offset)

        segments = segments.geometries().zip(distances).map(generate_line_middle_point)
    ...
    def process_short_line(line):
        ...
        return ee.Feature(geom).set("lineId", line.id()).set("offset", 0)
    ...
    def add_flow_accumulation(pt):
        ...
        return pt.set("flow_accumulation", ee.Number(flow_accumulation))

    def add_width(pt):
        width = (
            distance.reduceRegion(
                reducer=ee.Reducer.max(), geometry=pt.geometry(), scale=scale
            )
            .values()
            .get(0)
        )

        return pt.set("width", ee.Number(width).multiply(scale).multiply(2))
    ...
    def add_elevation(pt):
        ...
        return pt.set("elevation", ee.Number(elevation))
...
def api_get_rivers():
    ...
    # get ids
    upstream_catchment_ids = ee.List(selected_catchments.aggregate_array("HYBAS_ID"))

    logger.debug(
        "Number of catchments: %s" % repr(upstream_catchment_ids.size().getInfo())
    )

    # query rivers
    selected_rivers = rivers.filter(
        ee.Filter.inList("HYBAS_ID", upstream_catchment_ids)
...
def api_get_lakes():
    ...
    if id_only:
        ...
        return Response(
            json.dumps(ids.getInfo()), status=200, mimetype="application/json"
        )
...
def get_lake_by_id():
    ...
    lake = ee.Feature(
        ee.FeatureCollection(lakes.filter(ee.Filter.eq("Hylak_id", lake_id))).first()
    )
...
def get_lake_water_area(lake_id, scale):
    ...
    def get_monthly_water_area(i):
        ...
        if not scale:
            ...
            coords = ee.List(
                f.geometry().bounds().transform("EPSG:3857", 30).coordinates().get(0)
            )
        ...
        water_area = (
            ...
            .reduceRegion(ee.Reducer.sum(), f.geometry(), s)
            .values()
            .get(0)
        )
...
def api_get_feature_collection():
    ...
    features = features.filterBounds(region).map(clip_feature)
...
def get_feature_info():
    ...
    if function == "mosaic_elevation_datasets":
        ...
        image = ee.Image(
            dgds_functions.mosaic_elevation_datasets(datasets).select("elevation")
        )
    else:
    ...
    value = (
        image.sample(**{"region": ee.Geometry(bbox), "geometries": True, "scale": 10})
        .first()
        .getInfo()
    )
...
def get_image_collection_info():
    ...
    info = dgds_functions.get_image_collection_info(
...
def get_wms_url():
    ...
    info = dgds_functions.get_wms_url(
        image_id, "Image", band, datasets, function, min, max, palette
    )
...
def get_task_status():
    ...
    if operation_name:
        ...
        return (
            "either use request argument `operation_name` or `task_id`(deprecated)",
            400,
        )
...
def get_task_output():
    ...
    if task_id:
        operation_name = ee.data._cloud_api_utils.convert_task_id_to_operation_name(
            str(task_id)
        )
    try:
        op_dict = ee.data.getOperation(str(operation_name))
